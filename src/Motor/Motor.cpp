...
//    Serial.println(encoderTicks);
    ...
    // Perform RPM calculation at defined intervals
//    unsigned long elapsedTime = currentTime - lastUpdateTime;
//    if (elapsedTime >= sampleTimeMs) {
//        // Temporarily disable interrupts to safely read and reset encoderTicks
//        noInterrupts();
//        long ticks = encoderTicks;
//        encoderTicks = 0; // Reset tick count for the next period
//        interrupts();
//
//        // Calculate RPM based on ticks counted in the elapsed time
//        float rpm = ((float)ticks / pulsesPerRevolution) * (60000.0 / (float)elapsedTime);
//        currentRPM = rpmFilter.update(rpm); // Apply filtering to the RPM value
//
...
//        Serial.println(encoderTicks);
        ...
        lastUpdateTime = currentTime;
        // Here you can add any control logic that uses the calculated RPM,
        // for example, adjusting the motor power to reach a target RPM.
    }
}
